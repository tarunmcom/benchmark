...
all_benchmarks_completed_for_combo() {
  local model_path="$1" precision="$2" tp="$3"
  local model_name
    ...
    for input_len in "${INPUT_LENS[@]}"; do
      for output_len in "${OUTPUT_LENS[@]}"; do
        for rep in $(seq 1 "$REPEATS"); do
          ...
          local full_tag="${model_name}/${precision}_tp${tp}/${tag}"
...
REPEATS=1                        # Repetitions per combination
...
run_single_benchmark() {
  ...
  local tag="c${concurrency}_in${input_len}_out${output_len}_rep${rep}"
  local full_tag="$(model_short_name "$model_path")/${precision_label}_tp${tp_label}/${tag}"
...
run_benchmarks() {
  ...
  for entry in "${MODEL_CONFIGS[@]}"; do
    ...
    local model_name
    model_name=$(model_short_name "$model_path")
    # Disambiguate models with identical basenames (e.g. org-a/Llama-8B vs org-b/Llama-8B)
    ...
    for precision in "${precs[@]}"; do
      for tp in "${tp_sizes[@]}"; do
        ...
        if [[ -n "$RESUME_DIR" ]] && all_benchmarks_completed_for_combo "$model_path" "$precision" "$tp"; then
